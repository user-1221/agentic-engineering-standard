...
func (s *Storage) WritePackage(name, version string, data []byte) error {
	...
	return atomicWriteExclusive(path, data)
}
...
// atomicWriteExclusive writes data atomically and fails if the target already exists.
// Uses os.Link instead of os.Rename to enforce immutability without TOCTOU races.
func atomicWriteExclusive(path string, data []byte) error {
	...
	// os.Link fails if path already exists, preventing TOCTOU race
